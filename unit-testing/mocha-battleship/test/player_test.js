...
describe('PLAYER METHODS', function() {
    ...
    describe('placeShip', function() {
        ...
        it('should throw and error if no direction is specified', function() {
            var ship = player.ships[0];
            var coordinates = [0, 1];
            ...
            expect(handler).to.throw('You left out the direction! I neeeds it bad!');
        });
...
describe('COMPUTER PLAYER', function() {
   describe('randomGenerator', function() {
       var randomCoordinates = require('../game_logic/player_methods.js').randomCoordinates;
       var randomDirection = require('../game_logic/player_methods.js').randomDirection;
       
       it('should return coordinates between 0,0 and 9,9', function() {
            var coordinates = randomCoordinates();
           
           expect(coordinates[0]).to.be.below(10);
           expect(coordinates[1]).to.be.below(10);
           expect(coordinates[0]).to.be.at.least(0);
           expect(coordinates[1]).to.be.at.least(0);
       });
       
       it('should return a direction of horizontal or vertical', function() {
